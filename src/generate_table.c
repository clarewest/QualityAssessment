...
{
	int i,j,k,cov,cov2,h,b,l,good_h,good_b,good_l,end,ch,cb,cl;
	int total;
	...
	double resolution, CUTOFF;
  	char c,DSSP_Seq[1000],SS[1000],SS2[1000];		  /* The query protein's secondary structure			*/	
	FILE *input_fasta;
	FILE *library;
	FILE *input_dssp;
    int valid;
	...
	strcpy(AUX,argv[2]);
	input_dssp = fopen(strcat(AUX,".psipred_ss"),"r");
	...
	printf("%.2f\t%.2f\t%.2f",(float)j/start2,(float)numgood/valid,(float)cov/cov2);
	for(i=0;i<4;i++)
		printf("\t%.3f",(float)Numgood[i]/(Total[i]));
	printf("\t%.3f\t%.3f\t%.3f\t%.3f",(float)good_h/h,(float)good_b/b,(float)good_l/l,avg_length/(float)j);
    for(i=0;i<4;i++)
        printf("\t%.3f",(float)(Total[i])/valid);
    printf("\n");
	return 0;
}
